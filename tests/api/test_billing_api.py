...
async def test_billing_pricing_when_disabled(api_client, monkeypatch):
    ...
    import teardrop.routers.billing as app_module
...
async def test_billing_pricing_when_enabled_with_rule(api_client, monkeypatch):
    ...
    monkeypatch.setattr(app_module, "settings", mock_settings)
    monkeypatch.setattr("teardrop.routers.billing.get_current_pricing", AsyncMock(return_value=mock_rule))
...
async def test_billing_pricing_enabled_no_rule(api_client, monkeypatch):
    ...
    mock_settings.billing_enabled = True
    monkeypatch.setattr(app_module, "settings", mock_settings)
    monkeypatch.setattr("teardrop.routers.billing.get_current_pricing", AsyncMock(return_value=None))
...
async def test_billing_history_returns_list(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.get_billing_history", AsyncMock(return_value=mock_history))
...
async def test_billing_history_empty_list(api_client, monkeypatch):
    monkeypatch.setattr("teardrop.routers.billing.get_billing_history", AsyncMock(return_value=[]))
...
async def test_billing_history_scoped_to_authenticated_user(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.get_billing_history", mock_fn)
...
async def test_billing_balance_returns_balance(api_client, monkeypatch):
    monkeypatch.setattr(
        "teardrop.routers.billing.get_org_spending_config",
        AsyncMock(
            return_value={
                "org_id": "test-org-id",
...
async def test_billing_invoices_returns_items(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.get_invoices", AsyncMock(return_value=mock_invoices))
...
async def test_billing_invoices_empty_returns_null_cursor(api_client, monkeypatch):
    monkeypatch.setattr("teardrop.routers.billing.get_invoices", AsyncMock(return_value=[]))
...
async def test_billing_invoices_passes_cursor(api_client, monkeypatch):
    mock_fn = AsyncMock(return_value=[])
    monkeypatch.setattr("teardrop.routers.billing.get_invoices", mock_fn)
...
async def test_billing_invoice_by_run_found(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.get_invoice_by_run", AsyncMock(return_value=mock_invoice))
...
async def test_billing_invoice_by_run_not_found(api_client, monkeypatch):
    monkeypatch.setattr("teardrop.routers.billing.get_invoice_by_run", AsyncMock(return_value=None))
...
async def test_billing_invoice_scoped_to_authenticated_user(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.get_invoice_by_run", mock_fn)
...
async def test_billing_credit_history_returns_items(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.get_credit_history", AsyncMock(return_value=mock_entries))
...
async def test_billing_credit_history_empty(api_client, monkeypatch):
    monkeypatch.setattr("teardrop.routers.billing.get_credit_history", AsyncMock(return_value=[]))
...
async def test_billing_credit_history_operation_filter(api_client, monkeypatch):
    mock_fn = AsyncMock(return_value=[])
    monkeypatch.setattr("teardrop.routers.billing.get_credit_history", mock_fn)
...
async def test_usdc_requirements_returns_accepts_array(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.build_usdc_topup_requirements", MagicMock(return_value=[mock_req]))
...
async def test_usdc_requirements_billing_disabled_returns_503(api_client, monkeypatch):
    ...
    monkeypatch.setattr(
        "teardrop.routers.billing.build_usdc_topup_requirements",
...
async def test_usdc_topup_success_credits_org(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.verify_and_settle_usdc_topup", AsyncMock(return_value=mock_result))
    monkeypatch.setattr("teardrop.routers.billing.credit_usdc_topup", AsyncMock(return_value=2_000_000))
...
async def test_usdc_topup_duplicate_tx_returns_409(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.verify_and_settle_usdc_topup", AsyncMock(return_value=mock_result))
    monkeypatch.setattr("teardrop.routers.billing.credit_usdc_topup", AsyncMock(return_value=None))
...
async def test_usdc_topup_billing_disabled_returns_503(api_client, monkeypatch):
    ...
    monkeypatch.setattr(
        "teardrop.routers.billing.verify_and_settle_usdc_topup",
...
async def test_billing_pricing_includes_tool_overrides(api_client, monkeypatch):
    ...
    monkeypatch.setattr("teardrop.routers.billing.get_current_pricing", AsyncMock(return_value=mock_rule))
    monkeypatch.setattr(
        "teardrop.routers.billing.get_tool_pricing_overrides",
