"""Unit tests for tools.health — the per-tool circuit breaker."""
...
async def test_record_failure_trips_at_threshold(fake_redis, breaker_settings):
    with (
        patch("tools.health.get_redis", return_value=fake_redis),
        patch("tools.health.get_settings", return_value=breaker_settings),
    ):
...
async def test_breaker_disabled_is_no_op(fake_redis):
    ...
    s.tool_breaker_threshold = 3
    s.tool_breaker_window_seconds = 600
    with patch("tools.health.get_redis", return_value=fake_redis), patch("tools.health.get_settings", return_value=s):
...
async def test_fail_open_when_redis_unavailable(breaker_settings):
    with patch("tools.health.get_redis", return_value=None), patch("tools.health.get_settings", return_value=breaker_settings):
        assert (await tool_health.record_failure("tool-1")) is False
        assert (await tool_health.is_breaker_tripped("tool-1")) is False
        # Should not raise.
        await tool_health.record_success("tool-1")
