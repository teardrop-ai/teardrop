...
@pytest.mark.anyio
async def test_stream_emits_truncated_response_warning():
    graph = _EventGraph(
        [
            {
                "event": "on_chain_end",
                ...
                "data": {
                    "output": {
                        "task_status": "generating_ui",
                        "messages": [SimpleNamespace(content='{"partial":')],
                        "metadata": {"_synthesis_invalid": "truncated"},
                    }
                },
            },
            {"event": "on_chain_end", "name": "ui_generator", "data": {"output": {}}},
    ...
    assert {"type": "truncated_response", "message": warning_values[0]["message"]} == warning_values[0]
    assert "retry" in warning_values[0]["message"].lower()
