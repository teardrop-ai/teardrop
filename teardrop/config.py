...
class Settings(BaseSettings):
    ...
    agent_tool_max_calls_per_run: dict[str, int] = Field(
        default_factory=lambda: {"get_yield_rates": 2, "resolve_ens": 1, "get_protocol_tvl": 3},
        description=(
    ...
    rate_limit_mcp_discover_rpm: int = Field(
        default=10,
        description="Per-org rate limit for POST /mcp/servers/{id}/discover (requests per minute).",
    )
