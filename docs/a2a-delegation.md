...
When `MARKETPLACE_ENABLED=true`, an organization may separately opt into the public A2A Agent Endpoint Registry. `PUT /marketplace/agent-registration` is admin-only and validates an HTTPS base URL through the SSRF guard, fetches its `/.well-known/agent-card.json`, and requires the client-compatible `/message:send` endpoint. Registration is one row per organization and idempotently replaces that organization's previous URL; `GET` returns the authenticated organization's registration and `DELETE` unpublishes it. This registry is discoverability only: it does not add the endpoint to an organization's delegation allowlist, and publication is authorized by the organization admin rather than a cryptographic cross-domain ownership proof. The A2A client sends to the registered base URL plus `/message:send`.
