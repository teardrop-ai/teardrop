...
Asynchronous inbound tasks are persisted in Postgres and executed by a bounded per-process queue. Lease recovery is bounded and uses `FOR UPDATE SKIP LOCKED`; a process only recovers expired tasks owned by another process, so a live worker cannot reap its own task. Recovery never re-executes work: it records an interrupted projection and an immutable audit event. If the original lease owner later produces an authentic terminal result, the task projection may reconcile to that result, retaining the output and interruption reason without repeating billing or settlement. Tasks with no authentic result remain failed.
...
When bootstrap is disabled, its block and resource are omitted. If bootstrap pricing is unavailable, discovery still returns 200 with `bootstrap.accepts: []`, omits `amount_usdc`, and retains the token endpoint. Such responses require cache revalidation (`max-age=0`). Otherwise discovery cache lifetime is capped at the smaller of 300 seconds and `PRICING_CACHE_TTL_SECONDS` (never negative). Discovery amounts are advisory, not reserved quotes: request `POST /token` with `{"grant_type":"x402"}` and sign its current 402 requirements before paying. Configure `APP_BASE_URL` for canonical resource URLs in production.
